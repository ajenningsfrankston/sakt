...
class Model():
    ...
    def __init__(self,is_training,args,reuse=None):
        ...
        inputs = self._input_data = tf.compat.v1.placeholder(tf.int32, shape=(batch_size, num_steps-1))

        self.problems= tf.compat.v1.placeholder(tf.int32, shape=(batch_size, num_steps-1))
        self.target_id = target_id = tf.compat.v1.placeholder(tf.int32, shape=(None))
        self.target_correctness = target_correctness = tf.compat.v1.placeholder(tf.float32, shape=(None))
        self.Q= tf.compat.v1.placeholder(tf.float32, shape=(None,None))
        self.K= tf.compat.v1.placeholder(tf.float32, shape=(None,None))
        self.logits= tf.compat.v1.placeholder(tf.float32, shape=(None,None))
        ...
        with tf.compat.v1.variable_scope("encoder"):
               ...
               # key_masks = tf.expand_dims(tf.sign(tf.reduce_sum(tf.abs(x), axis=-1)), -1)
                ...
                self.enc += embedding(
                    tf.tile(tf.expand_dims(tf.range(tf.shape(input=x)[1]), 0), [tf.shape(input=x)[0], 1]),
        ...
        #
        self.enc = tf.reshape(self.enc, [tf.shape(input=x)[0] * (self.num_steps-1), args.hidden_units])
        #

        #

        sigmoid_w = tf.compat.v1.get_variable("sigmoid_w", [args.hidden_units, args.num_skills])
        sigmoid_b = tf.compat.v1.get_variable("sigmoid_b", [ args.num_skills])
        #
        ...
        #
        loss = tf.reduce_sum(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits = selected_logits, labels=target_correctness))
        self._cost = cost = loss
        tf.compat.v1.summary.scalar('loss', self._cost)
